...
def getCategory(url):
	...
	for item in j['items']:
		if 'teasers' in item:
			for teaser in item['teasers']:
				d = _parseTeaser(teaser)
				if d:
					l.append(d)
	return l

def _parseTeaser(teaser):
	d = False
	if teaser['_links']['target']['type'] != 'ExternalLink':
		d = {}
		tmpname = teaser['title'].split('|',1)[-1]
		d['_name'] = tmpname[0].upper()+tmpname[1:]
		d['_plot'] = teaser['description']
		d['url'] = teaser['_links']['target']['href']
		for image in teaser['image']['images']:
			if image['formatName'] == 'TeaserAufmacher':
				d['_thumb'] = image['imageUrl']
		d['_type'] = 'video'
		d['mode'] = 'playStream'
	return d
